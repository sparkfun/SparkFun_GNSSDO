...
//
// getSring is the core method called by all the other 'get' methods
// It calls performBackgroundTasks() to allow essential tasks to continue while waiting for user input
InputResponse getString(char *userString, uint8_t stringSize)
{
    ...
    while ((millis() - startTime) / 1000 <= menuTimeout)
    {
        ...
        performBackgroundTasks(); // Perform any essential background tasks while we wait for input
