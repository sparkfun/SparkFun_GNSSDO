...
!!! abstract "Directions"
	...
	This *quick start* guide is intended to help users get started with their GPSDO, without having to review the technical details of the product. It includes the minimum instructions to initially set up the GPSDO, depending on the primary interface that users would like to utilize:
	...
	**Ethernet**
	...
	: Instructions to connect the GPSDO to a network with an ethernet cable. This will provide the mosaic-T with network connectivity for its web server. Users can then pull up the configuration web page from any computer connected to that network.

	**mosaic-T USB-C**

	: For users that just want to interface and set up the GPSDO, directly through their computer.

	**ESP32 USB-C**

	: For users that want to configure the GPSDO firmware, using a terminal emulator on  their computer.
	...
	<figcaption markdown>
	Click on the tab of your preferred interface (below), to get started with the GPSDO.
	</figcaption>
	</figure>
...
=== "<H2>:material-ethernet:&nbsp;Ethernet</H2>"
	...
	The simplest way to get your GPSDO up and running is to connect it to your Ethernet network or an Ethernet port on your broadband router:
	...
	<figcaption markdown>
	[Ethernet Connections (PNG)](./assets/img/hookup_guide/Ethernet_Connections.png) for the GPSDO.
	</figcaption>
	</figure>

	**`1`** - **Connect the GNSS antenna**
	: * Inside your GPSDO kit, you will find the L1/L2/L5 GNSS "UFO" antenna. It has a TNC connection. Use the supplied TNC-SMA cable to connect the antenna to the mosaic-T **GNSS** SMA connection.
	...
	**`2`** - **Connect the GPSDO to your Ethernet network or router**
	...
	**`3`** - **Provide power**
	...
	<figcaption markdown>
	[mosaic T web page (PNG)](./assets/img/hookup_guide/T-web-page.png) for the GPSDO.
	</figcaption>
	</figure>

	!!! tip "Not working?"
		The following sections will help if your GPSDO is not working as expected:

		??? info "No power?"
			The red power (PWR) LED will light up when the GPSDO has power. If the PWR LED is off, make sure the wall adapter has power and the USB cable is connected.
...
=== "<H2>:material-usb-port:&nbsp;mosaic-T USB-C</H2>"
	...
	<figcaption markdown>
	[USB Connections (PNG)](./assets/img/hookup_guide/USB_Connections.png) for the GPSDO.
	</figcaption>
	</figure>
	...
	`3` - **Connect the GPSDO to your computer**
	...
	!!! tip "Not working?"
		...
		??? info "No power?"
			The red power (PWR) LED will light up when the GPSDO has power. If the PWR LED is off, make sure the USB cable is connected.
...
=== "<H2>:material-usb-port:&nbsp;ESP32 USB-C</H2>"
	...
	The SparkFun GPSDO contains an ESP32-WROVER microcontroller module. You can connect to its console through the **CONFIG ESP32** USB-C connector.
	...
	<figcaption markdown>
	[USB Connections (JPG)](./assets/img/hookup_guide/assembly-usb-esp32.jpg) for the GPSDO.
	</figcaption>
	</figure>
